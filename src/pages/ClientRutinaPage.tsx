                ...
                <div className={glassCard + ' p-5'}>
                  ...
                  <div className="flex items-start justify-between gap-4 pb-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                      {/* Botón WA compartir rutina */}
                      {wspTemplates['rutina-lista'] && clienteInfo?.phone && (() => {
                        const digits = clienteInfo.phone!.replace(/\D/g, '')
                        const waNum  = digits.startsWith('549') ? digits : '54' + digits
                        // Construye el detalle de ejercicios de la primera semana
                        const semana = rutina.semanas[0]
                        const detalleLineas = semana?.sesiones.map((ses, i) => {
                          const ejercicios = ses.bloques.flatMap(b => b.ejerciciosPlan).filter(e => e.nombre)
                          if (ejercicios.length === 0) return null
                          const lista = ejercicios.map(e =>
                            (e.series && e.repeticiones) ? `${e.nombre} ${e.series}×${e.repeticiones}` : e.nombre
                          ).join(', ')
                          return `Sesión ${i + 1}: ${lista}`
                        }).filter(Boolean) ?? []
                        const detalle = detalleLineas.join('\n')
                        // Solo mostrar el botón cuando hay ejercicios para compartir
                        if (!detalle) return null
                        const text = resolveWhatsappTemplate(wspTemplates['rutina-lista'], {
                          'cliente.nombre': clienteInfo.name,
                          'rutina.nombre':  rutina.nombre,
                          'rutina.detalle': detalle,
                        })
                        return (
                          <a
                            href={`https://wa.me/${waNum}?text=${encodeURIComponent(text)}`}
                            target="_blank" rel="noopener noreferrer"
                            title="Compartir rutina por WhatsApp"
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-emerald-200 dark:border-emerald-500/25 bg-emerald-50 dark:bg-emerald-500/[0.08] text-xs font-semibold text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-500/[0.15] transition-all"
                          >
