...
function diasStr(dias: string[]) {
  return dias.map(d => DIA_TO_SHORT[d.toLowerCase()] ?? d).join(' · ')
}
function todayStr() {
  const n = new Date()
...
export default function AusenciaPage() {
  ...
  const [fechaAusencia, setFechaAusencia] = useState(defaultFecha)
  ...
  // Fechas válidas para recuperación: próximas ocurrencias de cualquier turno (solo futuras)
  const fechasRecupValidas = useMemo(() => {
    if (turnos.length === 0) return []
    const dows = new Set<number>()
    turnos.forEach(t => shiftDows(t).forEach(d => dows.add(d)))
  ...
  async function handleCancelarRecup() {
    ...
    try {
      ...
      setAusenciaExistente(prev => prev && prev.recuperacion
        ? { ...prev, recuperacion: { ...prev.recuperacion, estado: 'CANCELADA' as const } }
        : prev)
      addToast({ type: 'success', message: 'Recuperación cancelada' })
    } catch {
  ...
  function Step1() {
    return (
      <div className="space-y-5">
        {/* Fecha — read-only en modo detalle, pill grid en creación */}
        <div className="space-y-1.5">
          ...
          {isDetail && ausenciaExistente ? (
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
              {fechasValidas.map(f => {
                const isSelected = fechaAusencia === f
                const today = todayStr()
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border text-sm transition-all duration-200 active:scale-[0.98] ${
            skipRecup
              ? 'bg-gray-50 dark:bg-white/[0.04] border-gray-200 dark:border-white/[0.08] text-gray-600 dark:text-[#8A8A9A]'
              : 'bg-gray-50 dark:bg-white/[0.04] border-gray-200 dark:border-white/[0.08] text-gray-400 dark:text-[#555]'
          }`}
        >
          <div className={`h-4 w-4 rounded-md border-2 flex items-center justify-center shrink-0 transition-all ${skipRecup ? 'bg-primary border-primary' : 'border-gray-300 dark:border-white/[0.2]'}`}>
  ...
  function Step2() {
    ...
    return (
      <div className="space-y-5">
        ...
        {ausenciaCreada && (
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-red-50 dark:bg-red-500/[0.08] border border-red-100 dark:border-red-500/20">
            ...
            <div>
              ...
              <p className="text-xs text-red-400/70 dark:text-red-400/50">
                {ausenciaCreada.fecha}
    ...
    <motion.div {...pageVariants} className="space-y-6">
      ...
      <div>
        <h1 className="text-3xl md:text-4xl font-black tracking-tighter text-gray-900 dark:text-white leading-none">
          {isDetail ? 'Detalle de ausencia' : 'Ausencia'}
        </h1>
