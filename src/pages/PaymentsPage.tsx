...
export default function PaymentsPage() {
  ...
  const filtered = useMemo(() => payments.filter(p => {
    ...
    return matchMethod && matchInvoiced
            ...
            <div className="flex items-center rounded-full border border-white/50 dark:border-white/10 bg-white/30 dark:bg-black/30 backdrop-blur-xl p-1 shadow-sm gap-1 shrink-0">
              ...
              {periodMode === 'day' ? (
                <div className="flex items-center px-1">
                  <input
                    type="date"
                    value={format(navDate, 'yyyy-MM-dd')}
                    max={format(today, 'yyyy-MM-dd')}
                    onChange={(e) => { if (e.target.value) setNavDate(new Date(e.target.value + 'T12:00:00')) }}
                    className="bg-transparent border-none outline-none text-xs font-bold text-gray-800 dark:text-gray-200 cursor-pointer [color-scheme:light] dark:[color-scheme:dark]"
                  />
                </div>
              ) : (
              ...
              return (
                <div key={p.id} onClick={() => navigate(`/payments/${p.id}`)}
                  ...
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    ...
                    <button
                      onClick={e => { e.stopPropagation(); if (!togglingIds.has(p.id) && can('payments', 'update')) toggleInvoiced(p) }}
                      disabled={togglingIds.has(p.id) || !can('payments', 'update')}
                      title={!can('payments', 'update') ? 'Sin permiso para editar pagos' : p.invoiced ? 'Quitar facturado' : 'Marcar como facturado'}
                      className="flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                ...
                <tr className="border-b border-white/20 dark:border-white/10 bg-gray-50/30 dark:bg-black/10">
                  {(['clientName', 'amount', 'paidAt', 'invoiced'] as const).map((key, idx) => (
                    <>
                      ...
                      {idx === 0 && (
                        <th className="px-6 py-4 text-left text-xs font-extrabold uppercase tracking-widest text-gray-500 dark:text-gray-400">CUIL</th>
                      )}
                    </>
                  ))}
                    ...
                    <tr key={p.id} onClick={() => navigate(`/payments/${p.id}`)}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3 group/name">
                          ...
                          <button onClick={e => handleCopy(e, `${p.id}-name`, p.clientName)} className="opacity-0 group-hover/name:opacity-100 transition-opacity text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
                            {copiedKey === `${p.id}-name` ? <Check size={12} className="text-emerald-500" /> : <Copy size={12} />}
                          </button>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1.5 group/cuil">
                          <span className="text-xs font-mono text-gray-500 dark:text-gray-400">{p.clientCuil || '—'}</span>
                          {p.clientCuil && (
                            <button onClick={e => handleCopy(e, `${p.id}-cuil`, p.clientCuil!)} className="opacity-0 group-hover/cuil:opacity-100 transition-opacity text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
                              {copiedKey === `${p.id}-cuil` ? <Check size={12} className="text-emerald-500" /> : <Copy size={12} />}
                            </button>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1.5 group/amount">
                          ...
                          <button onClick={e => handleCopy(e, `${p.id}-amount`, String(p.amount))} className="opacity-0 group-hover/amount:opacity-100 transition-opacity text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
                            {copiedKey === `${p.id}-amount` ? <Check size={12} className="text-emerald-500" /> : <Copy size={12} />}
                          </button>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-0.5 group/method">
                          <div className="flex items-center gap-1.5">
                            ...
                            <button onClick={e => handleCopy(e, `${p.id}-method`, METHOD_LABELS[p.method] ?? p.method)} className="opacity-0 group-hover/method:opacity-100 transition-opacity text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
                              {copiedKey === `${p.id}-method` ? <Check size={12} className="text-emerald-500" /> : <Copy size={12} />}
                            </button>
                          </div>
                        ...
                        {p.clientPhone ? (() => {
                          ...
                          return (
                            <a
                              ...
                              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-500/10 hover:bg-green-100 dark:hover:bg-green-500/20 border border-green-200 dark:border-green-500/20 transition-colors"
                            >
